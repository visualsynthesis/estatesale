...
Also provides scrape_feed() for broader Craigslist listings used by the live feed tab.
"""
...
# Feed searches — broader than the main scraper's estate-sale-only filter
FEED_SEARCHES = [
    {"url": f"{BASE_URL}/search/sss", "params": {"query": "estate sale", "sort": "date"}, "category": "estate sale"},
    {"url": f"{BASE_URL}/search/sss", "params": {"query": "moving sale", "sort": "date"}, "category": "moving"},
    {"url": f"{BASE_URL}/search/sss", "params": {"query": "antique", "sort": "date"}, "category": "antique"},
    {"url": f"{BASE_URL}/search/gms", "params": {"sort": "date"}, "category": "garage"},
]
...
def _parse_listing_for_feed(el) -> dict | None:
    """Parse a single CL listing element into a feed item."""
    title_el = el.select_one('.result-title, .posting-title, a.titlestring, .title')
    if not title_el:
        return None
    ...
    item = {"title": title, "url": "", "date": "", "description": "", "source": "craigslist"}
    ...
    if hood_el:
        desc_parts.append(hood_el.get_text(strip=True).strip('() '))
...
def scrape_feed() -> list[dict]:
    """Scrape broader Craigslist listings for the live feed tab.

    Returns a list of feed items with title, url, date, description,
    ...
    for search in FEED_SEARCHES:
        try:
            resp = requests.get(search["url"], params=search["params"],
                                headers=HEADERS, timeout=30)
            resp.raise_for_status()
        except requests.RequestException as e:
            logger.warning(f"Feed fetch failed for '{search['category']}': {e}")
            continue

        soup = BeautifulSoup(resp.text, 'lxml')
        listings = soup.select(
            '.result-row, .cl-static-search-result, li.result-info, .cl-search-result'
        )

        for el in listings:
            try:
                item = _parse_listing_for_feed(el)
                if not item:
                    continue
                ...
                logger.debug(f"Failed to parse feed listing: {e}")
                continue
